...
namespace FrostweepGames.Plugins.GoogleCloud.TextToSpeech
{
    ...
    public class GCTextToSpeech : MonoBehaviour
    {
        ...
        private readonly Dictionary<Enumerators.EffectsProfileId, string> EffectsProfiles = new Dictionary<Enumerators.EffectsProfileId, string>()
        {
            { Enumerators.EffectsProfileId.SMALL_HOME_SPEAKER, "small-bluetooth-speaker-class-device" },
            { Enumerators.EffectsProfileId.SMART_WATCH_OR_WEARABLE, "wearable-class-device" },
            { Enumerators.EffectsProfileId.SMARTPHONE, "handset-class-device" },
            { Enumerators.EffectsProfileId.HEADPHONES_OR_EAR_BUDS, "headphone-class-device" },
            { Enumerators.EffectsProfileId.SMART_HOME_SPEAKER, "medium-bluetooth-speaker-class-device" },
            { Enumerators.EffectsProfileId.HOME_ENTERTAINMENT_SYSTEM_OR_SMART_TV, "large-home-entertainment-class-device" },
            { Enumerators.EffectsProfileId.CAR_SPEAKER, "large-automotive-class-device" },
            { Enumerators.EffectsProfileId.INERACTIVE_VOICE_RESPONSE_SYSTEM, "telephony-class-application" },
        };
        ...
        public long Synthesize(string content, VoiceConfig voiceConfig, bool ssml = false, double pitch = 0.0, double speakingRate = 1.0, double sampleRateHertz = Constants.DEFAULT_SAMPLE_RATE, Enumerators.EffectsProfileId[] effectsProfileId = null, Enumerators.TimepointType[] timepoints = null, CustomVoiceParams customVoice = null)
        {
            ...
            var request = new PostSynthesizeBetaRequest()
            {
                audioConfig = new AudioConfig()
                {
                    ...
                    effectsProfileId = effectsProfileId == null ? new string[0] : effectsProfileId.Select(item => EffectsProfiles[item]).ToArray()
                },
                ...
                voice = new VoiceSelectionParams()
                {
                    ...
                    ssmlGender = voiceConfig.gender.ToString(),
        ...
        public long Synthesize(string content, VoiceConfig voiceConfig, bool ssml = false, double pitch = 0.0, double speakingRate = 1.0, double sampleRateHertz = Constants.DEFAULT_SAMPLE_RATE, Enumerators.EffectsProfileId[] effectsProfileId = null)
        {
            SynthesisInput synthesisInput;
