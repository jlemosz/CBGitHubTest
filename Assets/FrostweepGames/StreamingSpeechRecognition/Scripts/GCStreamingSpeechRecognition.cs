...
namespace FrostweepGames.Plugins.GoogleCloud.StreamingSpeechRecognition
{
    public class GCStreamingSpeechRecognition : MonoBehaviour
    {
		...
		private const int SampleRate = 48000; // LINEAR 16 frequency
		...
		public AudioClip _workingClip;

		public uLipSyncMicrophone uLipSyncMicrophone = null;
...
//			Initialize();	
		...
		private async void OnDestroy()
		{
			if (Instance != this || !_initialized)
				return;
			StreamingRecognitionFailedEvent -= VCAikataManager.Instance.HandleGoogleSTTError;
        ...
        private async void Update()
		{
			...
			if (uLipSyncMicrophone.isReady) _workingClip = uLipSyncMicrophone.clip;
		}
		...
		private IEnumerator CheckOnMicrophoneAndRunStream()
		{
			...
			while (!VCAccountManager.instance.LoggedIn && !Settings.instance.app.SkipLogin)
			{
				yield return null;
			}
		...
		private bool StartRecording()
		{
			...
			if (!uLipSyncMicrophone.isReady) VCAvatarManager.instance.OnChangeduLipSyncMic();

			StartCoroutine(func());
			IEnumerator func()
			{
				while (!uLipSyncMicrophone.isRecording)
				{
					yield return null;
				}

				_workingClip = uLipSyncMicrophone.clip;
                _previousSamplePosition = CustomMicrophone.GetPosition(uLipSyncMicrophone.device.name);
		...
		private void HandleRecordingData()
		{
			...
			_currentSamplePosition = CustomMicrophone.GetPosition(uLipSyncMicrophone.device.name);
			...
			if (CustomMicrophone.GetRawData(ref _currentAudioSamples, _workingClip))
			{
                if (_previousSamplePosition > _currentSamplePosition)
				{
					for (int i = _previousSamplePosition; i < _currentAudioSamples.Length; i++)
					{
						...
						if (_currentRecordedSamples != null && _currentAudioSamples != null)
						{
		...
		private async void WriteDataToStream()
		{
			...
			if (isRecording || (_currentRecordedSamples != null && _currentRecordedSamples.Count > 0))
			{
				...
				if (samplesChunk != null && samplesChunk.Count > 0)
				{
					...
					{
						if (ex.Message.Contains("Request had invalid authentication credentials"))
						{
							PlayerPrefs.SetString(ApplicationSettings.SK_CurrentCredentialKey, "");
							StreamingRecognitionFailedEvent?.Invoke($"Cannot proceed recognition due to: invalid authentication credentials");
							VCTranscriptionOptionManager.instance.OnFailKeyValidation();
							VCAikataManager.Instance.UIManager.OnFailKeyValidation();
						}
						else
						{
