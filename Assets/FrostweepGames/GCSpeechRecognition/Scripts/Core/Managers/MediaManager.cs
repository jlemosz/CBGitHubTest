...
namespace FrostweepGames.Plugins.GoogleCloud.SpeechRecognition
{
    public class MediaManager : IService, IMediaManager
    {
		...
		public void Update()
        {
			if (uLipSync.uLipSyncMicrophone.instance.isReady) _microphoneWorkingAudioClip = uLipSync.uLipSyncMicrophone.instance.clip;

			if (IsRecording)
            {
                _currentSamplePosition = CustomMicrophone.GetPosition(uLipSync.uLipSyncMicrophone.instance.device.name);
		...
		public void StopRecord(bool forceSaveCurrent=false)
		{
			...
			if (!DetectVoice || forceSaveCurrent)
			{
				LastRecordedRaw = _currentRecordingVoice.ToArray();
				LastRecordedClip = AudioConvert.Convert(LastRecordedRaw, _microphoneWorkingAudioClip.channels);			
			}
